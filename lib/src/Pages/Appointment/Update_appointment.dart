...
class UpdateAppointment extends StatefulWidget{
  final Map patient,appoints;

  UpdateAppointment({Key key,this.patient,this.appoints}):super(key: key);

  _updateAppointment createState()=>new _updateAppointment();
}
...
class _updateAppointment extends State<UpdateAppointment>
{
      ...
      await doctorService().getADoctor(data).then((value) {
        setState(() {
          if (value != "Fail") {
            ...
            if (widget.appoints["doctorid"] != null) {
              _doctor = widget.appoints["doctorid"];
            }
            // loaded = false;
          }
        });
  ...
  getDetails(){
    setState(() {
      selectedDate  =customFormat.parse(widget.appoints['date']);
      _controller= TextEditingController(text: widget.appoints['date'].toString());
      _selected = widget.appoints['specialized'];
      selectedday = DateFormat('EEEE').format(selectedDate);
      _scale .text =widget.appoints["severity"];
      var data=appoint_model(null, null, null, _selected, selectedday, null,null,null, null);
      getDoctors(data);
    });
  }
    ...
    return Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: [
            FlatButton(
              onPressed: (){
                _alertDialog(widget.appoints['refrenceID']);
              },
              child: Text(
                "Cancel",
                style: TextStyle(
        ...
        body: SmartRefresher(
          ...
          onRefresh: () async {
            ...
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(
                  builder: (context) => UpdateAppointment(patient: widget.patient,appoints: widget.appoints,)),
            );

          },
          child:Doctors!=null?SingleChildScrollView(
            child: Column(
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    // widgetPage().numberField("Severity [1 - 5] (if 1- Most Severe)", true, _scale),
